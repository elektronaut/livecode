...
VALUE mLivecode = Qnil;
VALUE mLivecodeMIDI = Qnil;
VALUE mCoreMIDI = Qnil;
VALUE mCoreMIDIAPI = Qnil;

VALUE cEndpoint = Qnil;
VALUE cOutputPort = Qnil;
...
// A struct that contains the endpoint
typedef struct RbEndpoint_t {
    MIDIEndpointRef endpoint;
} RbEndpoint;

// A struct that contains the output port
typedef struct RbOutputPort_t {
...
/*
 *
 * RbEndpoint related methods
 *
 */

static void endpoint_free(void* ptr)
{
	if(ptr != NULL)
		free(ptr);
}
...
{
    RbEndpoint* endpoint = (RbEndpoint*) malloc(sizeof(RbEndpoint));
    endpoint->endpoint = NULL;
    
    VALUE obj;
    obj = Data_Wrap_Struct(klass, 0, endpoint_free, endpoint);
    
    return obj;
}

static VALUE endpoint_initialize(VALUE self)
{
    return self;
}
...
/*
 *
 * RbOutputPort related methods
 *
 */

static void outputport_free(void* ptr)
{
	if(ptr != NULL)
		free(ptr);
}

static VALUE outputport_alloc(VALUE klass)
{
    RbOutputPort* port = (RbOutputPort*) malloc(sizeof(RbOutputPort));
    port->output_port = NULL;
    
    VALUE obj;
    obj = Data_Wrap_Struct(klass, 0, outputport_free, port);
    
    return obj;
}

static VALUE outputport_initialize(VALUE self)
{
    return self;
}
...
void Init_coremidi()
{
	...
	mLivecode = rb_define_module("Livecode");
	mLivecodeMIDI = rb_define_module_under(mLivecode, "MIDI");

	mCoreMIDI = rb_define_module_under(mLivecodeMIDI, "CoreMIDI");
	...
	// Define CoreMIDI::API::Endpoint class
	cEndpoint = rb_define_class_under(mCoreMIDIAPI, "Endpoint", rb_cObject);
	rb_define_alloc_func(cEndpoint, endpoint_alloc);
	rb_define_method(cEndpoint, "initialize", endpoint_initialize, 0);

	// Define CoreMIDI::API::OutputPort class
	cOutputPort = rb_define_class_under(mCoreMIDIAPI, "OutputPort", rb_cObject);
	rb_define_alloc_func(cOutputPort, outputport_alloc);
	rb_define_method(cOutputPort, "initialize", outputport_initialize, 0);
